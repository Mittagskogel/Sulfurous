...
stdin.addListener("data", function(d) {
  ...
  setInterval(function(){
	...
	for(var i = 0;i < Object.keys(CLOUDSAVE).length;i++){
			...
			var ttl = CLOUDSAVE[Object.keys(CLOUDSAVE)[i]].ttl;
			...
			console.log("Autosaved project "+Object.keys(CLOUDSAVE)[i]);	
			...
			CLOUDSAVE[Object.keys(CLOUDSAVE)[i]].ttl = ttl;
  ...
  setInterval(function(){
	...
	for(var i = 0;i < Object.keys(CLOUDSAVE).length;i++){
		...
		if(CLOUDSAVE[Object.keys(CLOUDSAVE)[i]].ttl == 0){
			...
			console.log("Removed project "+Object.keys(CLOUDSAVE)[i]+" from the RAM");
  ...
  var loadJSON = function(projectID){
	  ...
	  try{
		  ...
		  CLOUDSAVE[projectID] = {"vars":{},"lists":{}};
		  ...
		  fs.writeFile('data/'+projectID+'.json', JSON.stringify(CLOUDSAVE[projectID]), function(err) {
			...
			console.log("Created new Cloud Project: "+projectID);
			}); 
	  }
	console.log("loadJSON for project: "+projectID);
  }
   ...
   socket.on('getReq', function (data) {
		...
		if(typeof CLOUDSAVE[data.projectID] == 'undefined'){
			...
			loadJSON(data.projectID);
				...
				if(typeof data.sulfCloudListsChanged != 'undefined'){
							
						//typeof Object.keys(data.sulfCloudListsChanged)[0] != 'undefined'
		
					for(var i = 0;i<Object.keys(data.sulfCloudListsChanged).length;i++){
						
						
						
					
						
						console.log(data.sulfCloudListsChanged[Object.keys(data.sulfCloudListsChanged)[i]]);
						
						CLOUDSAVE[data.projectID].lists[Object.keys(data.sulfCloudListsChanged)[i]] = data.sulfCloudListsChanged[Object.keys(data.sulfCloudListsChanged)[i]];
		...
		socket.emit('getRes', {"vars" : CLOUDSAVE[data.projectID].vars, "lists": CLOUDSAVE[data.projectID].lists} );
